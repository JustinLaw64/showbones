...
function showbones.db_load() -- Loads entire showbones database
	...
	print("[" .. showbones.modname .. "] Reading from database")
	if file then
...
function showbones.db_save(data) -- Merges new data with stored data and saves to db
	...
	if file then
		...
		file:close()
		print("[" .. showbones.modname .. "] Writing to database")
	end
end
...
function showbones.update_bones_locations(pos, player_name) 
	...
	if table.getn(bones_locations) >= showbones.showbones_limit then
		...
		print("[" .. showbones.modname .. "]" .. player_name .. " has too many bones! Forgetting")
...
function showbones.get_player_table(player_name) -- grabs player table by passing a player_name
	if showbones_temp[player_name] then
		...
		--return player_table
		return showbones_temp[player_name]
	else
...
function showbones.announce_bones(pos, player_name) -- verify bones at pos
	...
	if node.name == "bones:bones" then
		...
		print("[" .. showbones.modname .. "] " .. player_name .. " died and left us bones! " .. minetest.pos_to_string(pos)) -- to console for debugging
		return pos
	end
end
...
function showbones.show_hud(player_name) 
	...
	elseif total_bones_locations >= showbones.showbones_limit then
		message = "[".. showbones.modname .. "] Your Bones are now shown as " .. total_bones_locations .. " waypoints.\nUnclaimed Bones beyond " .. showbones.showbones_limit .. " will be lost from the tracker."
	end
	
	minetest.chat_send_player(player_name, message)
	return true, ""
end
function showbones.toggle_hud(player_name)
	local player = minetest.get_player_by_name(player_name)
...
function showbones.remembers_bones_pos(player_name, pos)
	showbones_temp[player_name] = showbones.get_player_table(player_name)
	local bones_locations = showbones_temp[player_name]["bones_locations"]
	local r = false
	for k, v in pairs(bones_locations) do
		if v.x == pos.x and v.y == pos.y and v.z == pos.z then 
			r = true
		end
	end
...
function showbones.chat_notify(owner, bones_breaker)
	local messages = { -- add more messages if you want
		...
		"Something in your bones has been taken."
	}
...
function showbones.bones_removed(pos, player)      -- called by bones:bones on_punch
	...
	if showbones.remembers_bones_pos(showbones_owner, pos) then
...
if minetest.get_modpath("unified_inventory") then
	unified_inventory.register_button("showbones", {
		type = "image",
		image = minetest.inventorycube(
			"(bones_top.png^[transform2)",
			"bones_front.png",
			"bones_side.png"
		),
		tooltip = "Show your lost bones",
		action = function(player)
			local player_name = player:get_player_name()
			showbones.toggle_hud(player_name)
		end,
	})
end
]]
